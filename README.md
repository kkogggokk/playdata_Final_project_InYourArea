# 📝 Table of Contents
- [PART 1. 데이터수집](#PART-1.-데이터수집)
- [PART 2. 라벨링](#PART-2.-라벨링)
    - [## 2.1 라벨링 기준](##-2.1-라벨링-기준)
- [PART 3. 모델 : YOLOv5 사용](#-PART-3.-모델:YOLOv5-사용)
    - [3.1 특징](##-3.1-특징)
...
- 따라서 그룹을 "블랙핑크"로 변경하였다. 크롤링한 사진에는 기사사진, 광고사진, 무대사진 등 다양한 사진이 존재했으며 무대 영상에 대한 예측률이 **%정도 나왔다. 
- 예측할 영상과 가장 유사한 데이터를 사용해야 좋을 결과가 나오리라 판단하여 무대 직캠 영상을 15프레임마다 캡처해 이미지 데이터를 수집하였다. (capture.py)
